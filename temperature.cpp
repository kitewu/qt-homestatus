...
void Temperature::initVariables()
{
...
void Temperature::drawOuterCircle(QPainter *painter)
{
    painter->save();
...
void Temperature::drawInnerCircle(QPainter *painter)
{
    painter->save();
...
void Temperature::drawMark(QPainter *painter)
{
    painter->save();
...
void Temperature::drawCoverBall(QPainter *painter)
{
    painter->save();
...
void Temperature::drawTextRect(QPainter *painter)
{
    painter->save();
    ...
    QString strValue;
    strValue=tr("%1").arg(m_value);//设置文本显示的值
    painter->drawText(textRect,Qt::AlignHCenter|Qt::AlignVCenter,strValue);
    painter->restore();
}

void Temperature::drawCoverCircle(QPainter *painter)
{
    painter->save();
...
void Temperature::drawColorPie(QPainter *painter)
{
    painter->save();
...
void Temperature::drawIndicator(QPainter *painter)
{
    painter->save();

    painter->translate(m_center);
    qreal increment=(qreal)270/100;
    qreal changedAngle=45+increment*(m_currentValue+40);
...
void Temperature::resetVariables(QPainter *painter)
{
    m_outerRadius=width()>height() ? height()/2: width()/2;
    m_innerRadius=m_outerRadius/8*7;
    m_coverCircleRadius=m_outerRadius/8*4+10;
...
void Temperature::setValue(qreal value)
{
    if(value>m_value)
    {
        m_bReverse=false;
